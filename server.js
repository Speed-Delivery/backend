...
io.on("connection", (socket) => {
  ...
  socket.on('getUserParcels', async (userId) => {
    try {
      ...
      const userParcels = await Parcel.find({ 'sender.user': userId }).populate('sender.user');
      console.log(userParcels);
      socket.emit('userParcels', userParcels);
    } catch (error) {
      // Handle error appropriately
      console.error('Error fetching user parcels:', error);
      socket.emit('userParcelsError', { message: 'Error fetching user parcels' });
    }
  });
  ...
  socket.on('getUserLockers', async (userId) => {
    try {
      ...
      socket.emit('userLockersError', { message: 'Error fetching user lockers' });
    }
  });
